...
describe("ERC20 functions", function () {
  ...
  describe("Transfers", function(){
    ...
    it("Should revert minting if you are not a bridge or an owner", async function() {
      await expect(contract.connect(addr2).mint(addr2.address, 10)).to.be.revertedWith("Not a bridge or an owner");
    });

    it("Should revert burning if you are not a bridge or an owner", async function() {
      await expect(contract.connect(addr2).burn(addr2.address, 10)).to.be.revertedWith("Not a bridge or an owner");
    });
  });
});